...
import otros.DataSistema;


public class Sesion implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        DataSistema.UserInformation();
        DataSistema.cargarDatosProductos();
        DataSistema.cargarDatosClientes();
        DataSistema.OrderInformation();
        
        clientes = new ArrayList();
        for (Cliente c : DataSistema.clientes1) {
            clientes.add(c);
        }
        productos = new ArrayList();
        for (Producto p : DataSistema.productos1) {
            productos.add(p);
        }
    ...
    private void login(ActionEvent event) {
        ...
        if (valida == true) {
             ...
             FXMLLoader loader = new FXMLLoader(getClass().getResource("/vista/InicioVista.fxml"));
                ...
                Stage myStage = (Stage) this.txtLogin.getScene().getWindow();
                myStage.close();
         }catch(IOException e){
